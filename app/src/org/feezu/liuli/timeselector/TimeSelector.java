...
public class TimeSelector {
    ...
    public void show(Date date) {
        ...
        year_pv.setSelected(fomatTimeUnit(selectedYear));
        month_pv.setSelected(fomatTimeUnit(selectedMonth));
        day_pv.setSelected(fomatTimeUnit(selectedDay));
        hour_pv.setSelected(fomatTimeUnit(selectedHour));
        minute_pv.setSelected(fomatTimeUnit(selectedMinute));
    ...
    private void monthChange() {
        ...
        int selected = selectedCalender.get(Calendar.MONTH) + 1;
        String selectedStr = fomatTimeUnit(selected);
        int index = month.indexOf(selectedStr);
        if(index == -1){
        	index = 0;
        }
    ...
    private void dayChange() {
        ...
        int selected = selectedCalender.get(Calendar.DAY_OF_MONTH);
        String selectedStr = fomatTimeUnit(selected);
        int index = day.indexOf(selectedStr);
        if(index == -1){
        	index = 0;
        }
    ...
    private void hourChange() {
        if ((scrollUnits & SCROLLTYPE.HOUR.value) == SCROLLTYPE.HOUR.value) {
            ...
            int selected = selectedCalender.get(Calendar.HOUR_OF_DAY);
            String selectedStr = fomatTimeUnit(selected);
            int index = hour.indexOf(selectedStr);
            if(index == -1){
            	index = 0;
            }
    ...
    private void minuteChange() {
        if ((scrollUnits & SCROLLTYPE.MINUTE.value) == SCROLLTYPE.MINUTE.value) {
            ...
            int selected = selectedCalender.get(Calendar.MINUTE);
            String selectedStr = fomatTimeUnit(selected);
            int index = minute.indexOf(selectedStr);
            if(index == -1){
            	index = 0;
            }
